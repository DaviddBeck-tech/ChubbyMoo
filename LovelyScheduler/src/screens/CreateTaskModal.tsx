...
export default function CreateTaskModal({ isVisible, onClose, selectedDateString, navigation, route }: any) {
  const isScreen = navigation !== undefined;
  const activeVisible = isScreen ? true : isVisible;
  const activeClose = isScreen ? () => navigation.goBack() : onClose;
  const activeDateString = isScreen ? (route.params?.selectedDateString || new Date().toISOString().split('T')[0]) : selectedDateString;
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            <Text style={styles.modalSubtitle}>Được lên lịch cho {activeDateString}</Text>
